...
@section('title', 'Create Task Priorities')

@section('content')
<div class="tp-container">
    <div class="tp-header">
        <h1 class="tp-title">Create Task Priorities</h1>
    ...
    <form method="POST" action="{{ route('task-priorities.store') }}" id="tp-create-form">
        @csrf

        <div class="tp-card tp-card--section" aria-labelledby="instruction-section">
            <div class="tp-card-header">
                <div class="tp-card-title">Instruction</div>
                <div class="tp-card-sub">Select the instruction you are prioritizing</div>
            </div>
            <div class="tp-grid">
                <div class="tp-field">
                    <label for="instruction_id">Instruction Title <span class="req" aria-hidden="true">*</span></label>
                    <select id="instruction_id" name="instruction_id" required aria-describedby="instruction_help">
                        <option value="">Select instruction...</option>
                        @foreach($instructions as $inst)
                            <option value="{{ $inst['id'] }}" @selected(old('instruction_id') == $inst['id'])>
                                {{ $inst['title'] }} — {{ $inst['sender_name'] }}
                            </option>
                        @endforeach
                    </select>
                    <small id="instruction_help" class="tp-help">Only instructions assigned to you are listed.</small>
                </div>
            </div>
        </div>
        ...
        <div class="tp-card tp-card--section" aria-labelledby="priority-items-section">
            <div class="tp-card-header">
                <div>
                    ...
                    <div class="tp-card-sub">Add one or more items. Dates must be valid and realistic.</div>
                </div>
...
(function() {
  ...
  // Initialize existing server-rendered old items
  $$('#tp-items [data-item]').forEach(initItem);
  if ($$('#tp-items [data-item]').length === 0) {
    // Add one default item on load if none
    addItem();
  }

  // Basic client-side validation: ensure at least one item exists
  form.addEventListener('submit', (e) => {
    if ($$('.tp-item').length === 0) {
      e.preventDefault();
      alert('Please add at least one priority item.');
    }
